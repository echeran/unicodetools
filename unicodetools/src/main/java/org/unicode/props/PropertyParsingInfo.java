        ...
        if (intRange.string != null) {
            ...
            for (int codepoint = intRange.start; codepoint <= intRange.end; ++codepoint) {
                try {
                    if (hackHangul) {
                        String fullDecomp =
                                NFD.getDecomposition(codepoint); // use ICU for Hangul decomposition
                        ...
                        PropertyUtilities.putNew(
                                ...
                                merger); // use ICU for Hangul Name construction, constant
                    } else {
                    ...
                    String msg =
                            String.format(
                                    "%s: %04X..%04X  %s",
                                    property, intRange.start, intRange.end, string);
